//
// Created by shvarev on 28.12.22.
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int digsign(int argc, tchar *argv[]){
    ...
    enum { do_nothing, do_new, do_show, do_verify, do_cert, do_repo_add, do_repo_rm,
        do_repo_check, do_repo_ls, do_p7b_create, do_p7b_ls, do_p7b_split , create_sign, do_ver} work = do_nothing;
    ...
    struct file ofp, aaa;
/*
    int next_option = 0, exitcode = EXIT_FAILURE, exit_status = EXIT_FAILURE;
    enum {do_nothing, smth, create_sign, do_ver, do_new} work = do_nothing;
    */
    ...
    do {
        next_option = getopt_long(argc, argv, "hs:d:v:p:n:", long_options, NULL);
    ...
    /*error = ak_asn1_import_from_file( root = ak_asn1_new(), "", NULL );
    printf("%d\n", error);
    error = ak_certificate_import_from_asn1( &certzxc, NULL, root );
    printf("%d\n", error);*/

    if( !aktool_create_libakrypt( )) return EXIT_FAILURE;
/*
    if( ak_certificate_import_from_file( &certzxc, NULL, "aktool256_request.csr" ) != ak_error_ok ) {
    ...
    */
    ...
    //ak_asn1_import_from_file( root = ak_asn1_new(), "12ok.csr", NULL );
    //ak_request_import_from_asn1( &req, root );
    //size_t ts = ak_hash_get_tag_size( req.vkey.ctx );

    //printf("%s", ak_ptr_to_hexstr(req.opts.signature, 64, ak_false), 64 );
    ...
    if( work == do_ver ) {
        ...
        /*if( ak_certificate_import_from_file( &certzxc, NULL, filept ) != ak_error_ok ) {
            aktool_error(_("incorrect reading of CA certificate (see --ca-cert option)"));
        }*/
        //ak_verifykey_create_from_signkey( &vk, &lk );

        ak_asn1_import_from_file( root = ak_asn1_new(), "ok.csr", NULL );
        ak_request_import_from_asn1( &req, root );
